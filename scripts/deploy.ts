...
/**
 * Write a block of `KEY=value` pairs into an env file, replacing what is already there.
 *
 * This used to `appendFile`, which is only correct the first time. Every later run added a
 * second copy of all seven contract IDs while the first copy stayed in place, and dotenv
 * resolves duplicates to the *first* match — so re-running a deploy appeared to succeed and
 * silently left the application pointed at the previous deployment's contracts. The frontend
 * `.env.local` had accumulated five stacked generations before the bug was noticed.
 *
 * So: drop every line that this run owns (the marker comment and any key being written),
 * keep everything else the developer put in the file, and add the new block at the end.
 */
async function writeEnvBlock(
  file: string,
  header: string,
  ...
  const kept = existing
    .split("\n")
    .filter((line) => {
      if (line.startsWith("# ZizaLend contracts —")) return false;
      const key = /^\s*([A-Z0-9_]+)\s*=/.exec(line)?.[1];
      return key === undefined || !owned.has(key);
    })
    .join("\n")
    .replace(/\n{3,}/g, "\n\n")
    .replace(/^\n+/, "")
...
async function main() {
  ...
  if (
    await readBool(nftContractId, "is_authorized_minter", [managerContractId])
  ) {
    console.log(
  ...
  for (const [label, contractId, governorId] of [
    ...
    if ((await readAddress(contractId, "get_governance")) === governorId) {
      console.log(`  ${label}.set_governance(...) — already wired, skipping`);
    } else {
  ...
  if (config.parameters) {
    ...
    const applyI128 = async (
      ...
      if (current !== undefined && scValToI128(current) === BigInt(value)) {
  ...
  await writeEnvBlock(path.join(__dirname, "../frontend/.env.local"), header, {
  ...
  await writeEnvBlock(path.join(__dirname, "../backend/.env"), header, {
