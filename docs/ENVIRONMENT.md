...
| `NEXT_PUBLIC_DEMO_DATA`            | ✓   | ✓       | —    | `false`                                   | Set to `true` on a deployment whose database was seeded by `backend/src/seed`, so the interface labels the rows as demo data. Must not be set in production. See `docs/DEMO-DATA.md`. | `frontend/src/app/components/global_ui/DemoDataBanner.tsx` |
