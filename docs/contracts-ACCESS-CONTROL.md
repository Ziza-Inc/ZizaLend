...
| `repay(borrower, loan_id, amount)` | `borrower` | No | Splits `amount` proportionally into principal/interest/latefee; bumps `principal_paid`, decrements balances; if fully repaid: marks `Loan.status = Repaid`, decrements `BorrowerLoanCount`, **then** transfers borrower → pool + releases collateral + credits the score via `update_score` (skipped when the payment is below the NFT's configured minimum, so a high floor cannot fail the repayment) | `LoanRepaid`, optionally `LateFeeCharged`, optionally `ScoreReportSkipped` | Closes loan; increased credit | CEI: full state commit precedes transfers; reentrancy on same loan hits `LoanNotActive`. The score write is best-effort and cannot fail a payment that has already been made |
...
| `check_default(loan_id)` / `check_defaults(loan_ids)` | admin | No | Marks `Loan.status = Defaulted`; seizes collateral; **decrements REMITTANCE NFT SCORE** | `LoanDefaulted`, optionally `DefaultReportSkipped` | Closes loan; degrades credit | CEI: score penalty applied after state commit. Both entry points share one pipeline. The batch isolates per loan — a missing loan, a terminal loan, or a due date whose default window overflows is skipped rather than failing the batch — and the NFT score write is best-effort, so a misconfigured NFT cannot stop collateral being seized on every other loan |
...
| `emergency_withdraw(provider, token, shares)` | `provider` | No | Same as `withdraw`, **bypasses** cooldown and pause, but enforces the minimum hold time unconditionally | `Withdraw` (same event) | Exit hatch when admin pause is in flight. Uses `assert_share_held_for_minimum_ledgers`, not the cooldown-deferring variant: because this path skips the cooldown check, the deferring variant would leave it with no flash-loan guard at all | Reentrancy-safe (bypasses `assert_not_paused`) |
...
| `adjust_outstanding(token, delta)` | configured LoanManager contract | Yes — `require_auth` on the stored LoanManager | `TotalOutstanding(token)` | (none) | LoanManager bookkeeping (net-delta form of the two above). Refuses with `LoanManagerNotSet` when none is configured, and with `OutstandingOverflow` / `OutstandingUnderflow` when the delta would leave the representable, non-negative range. Any other caller aborts in the host's authorisation check, which cannot surface as a code because `require_auth` traps | CEI-safe |
| `set_governance(governance)` | admin | No | `Governance` | `GovernanceSet` | From this point `set_admin` requires the governance contract's auth | n/a |
| `propose_admin` / `accept_admin` | admin / proposed admin | No | `ProposedAdmin`, `Admin` | `AdminProposed`, `AdminTransferred` | Two-step admin hand-off; works with or without governance configured | n/a |
| `set_admin(new_admin)` | configured governance, else current admin | No | `Admin`, clears `ProposedAdmin` | `AdminTransferred` (via = `admin` \| `governance`) | Hands admin role; this is the entry point `MultisigGovernance::finalize_admin_transfer` invokes | n/a |
| `upgrade(wasm_hash)` | admin | No | `Version` | `ContractUpgraded` + WASM swap | Contract bytecode identity | n/a |
