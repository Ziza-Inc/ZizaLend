# ZizaLend Backend API

Express.js backend service for the ZizaLend platform, providing API endpoints for credit scoring, remittance simulation, and NFT metadata management.
...
```bash
# Build image
docker build -t ZizaLend-backend .

# Run container
docker run -p 3001:3001 --env-file .env ZizaLend-backend
```
